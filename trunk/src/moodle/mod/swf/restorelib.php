<?php //$Id: restorelib.php,v 1.13 2006/09/18 09:13:04 moodler Exp $
    ...
    function swf_decode_content_links_caller($restore) {
		...
		//
		if ($swfs = get_records_sql ("SELECT s.id, s.intro, s.swfurl, s.xmlurl, s.skin, s.configxml
            ...
            foreach ($swfs as $swf) {
				...
				//
				if ($r_intro != $intro || $r_swfurl != $swfurl || $r_xmlurl != $xmlurl || $r_skin != $skin || $r_configxml != $configxml) {
                    //Update record
