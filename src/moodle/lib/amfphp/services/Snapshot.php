...
* @version $Id: Snapshot.php,v 1.0 2011/04/21 matbury Exp $
	...
	public function amf_save_snapshot($obj)
	{
		...
		*/
		...
		if ($capabilities->is_logged_in && $capabilities->view_own_grades)
		{
			...
			$swf_filename = str_replace($needles,'_',$USER->lastname.'_'.$USER->firstname).'.'.$obj['imagetype']; // file name e.g. John_Smith.jpg
			//$swf_filename = str_replace($needles,'_',$USER->lastname.'_'.$USER->firstname).'_'.date('Y\_M\_dS\_h\-i\-s').'.'.$obj['imagetype']; // doesn't overwrite previously saved images
			
			// Build path to course files directory and subdirectory /snapshots/99_Concept_map_activity/
			$dataroot_path = $CFG->dataroot.'/'.$capabilities->course.'/snapshots/'.$capabilities->cmid.'_'.$swf_dirname.'/';
			$moodledata_path = $CFG->wwwroot.'/file.php/'.$capabilities->course.'/snapshots/'.$capabilities->cmid.'_'.$swf_dirname.'/';
			
			// If, for some reason, you can't write to moodledata, you can write to an alternative directory, e.g. http://example.com/moodle/snapshots/[courseid]/...
			
			// Build path to alternative files directory and subdirectory /snapshots/[courseid]/99_Concept_map_activity/
			//$dataroot_path = $CFG->dirroot.'/snapshots/'.$capabilities->course.'/'.$capabilities->cmid.'_'.$swf_dirname.'/';
			//$moodledata_path = $CFG->wwwroot.'/snapshots/'.$capabilities->course.'/'.$capabilities->cmid.'_'.$swf_dirname.'/';
			
			// If necessary, create directory with SWF Activity Module instance name, e.g. /home/sites/example.com/moodledata/99/snapshots/Concept_map_activity/
			try {
			...
			{
				$swf_return->result = 'FILE_NOT_WRITTEN';
				$swf_return->imageurl = $dataroot_path.' course files directory is not writeable.'; // To be deprecated - Don't send messages on this parameter
			...
			if($result)
			{
				...
				$swf_return->message = 'Your image was successfully saved.';
				return $swf_return;
			} else {
				$swf_return->result = 'FILE_NOT_WRITTEN';
				$swf_return->imageurl = $moodledata_path_filename_time;
				$swf_return->message = 'There was a problem. Your image was not saved.';
				return $swf_return;
			}
		}
		$swf_return->result = 'NO_PERMISSION';
		$swf_return->imageurl = 'You do not have permission to save images.'; // To be deprecated - Don't send messages on this parameter
		$swf_return->message = 'You do not have permission to save images.';
		return $swf_return;
	}
