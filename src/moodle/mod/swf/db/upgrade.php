...
/// Please, delete these comment lines once this file starts handling proper upgrade code.
...
/// First example, some fields were added to the module on 20090900
    ...
    /// Launch add field course
		...
		// Add fullbrowser field
		if ($result && $oldversion < 2011020200) {
		/// Define field fullbrowser to be added to swf
			$table = new XMLDBTable('swf');
			$field = new XMLDBField('fullbrowser');
			$field->setAttributes(XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, null, null, 'false', 'height');
		/// Launch add field course
			$result = $result && add_field($table, $field);
		}
		...
		/// Define field fullbrowser to be added to swf
			...
			$field->setAttributes(XMLDB_TYPE_TEXT, 'medium', null, null, null, null, null, null, 'fullbrowser');
		/// Launch add field course
			$result = $result && add_field($table, $field);
		}
		
		// Add sequenced field
		if ($result && $oldversion < 2012033001) {
		/// Define field sequenced to be added to swf
			$table = new XMLDBTable('swf');
			$field = new XMLDBField('sequenced');
			$field->setAttributes(XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, null, null, 'false', 'gradepass');
		/// Launch add field course
			$result = $result && add_field($table, $field);
		}
		
		// Add allowreview field
		if ($result && $oldversion < 2012040400) {
		/// Define field fullbrowser to be added to swf
			$table = new XMLDBTable('swf');
			$field = new XMLDBField('allowreview');
			$field->setAttributes(XMLDB_TYPE_CHAR, '20', null, XMLDB_NOTNULL, null, null, null, 'true', 'gradepass');
		/// Launch add field course
			$result = $result && add_field($table, $field);
		}
		
		// Add sequenced field
		/*if ($result && $oldversion < 2011100200) {
		/// Define field fullbrowser to be added to swf
			$table = new XMLDBTable('swf');
			$field = new XMLDBField('sequenced');
			$field->setAttributes(XMLDB_TYPE_CHAR, '5', null, XMLDB_NOTNULL, null, null, null, 'false', 'gradepass');
		/// Launch add field course
			$result = $result && add_field($table, $field);
		}*/
